...
def project_py(score_threshold):
    ...
    # 假日女班（0700~1330） -> 總和: 18
    women_weekend = [int(we) for we in all_weekend[0:14]]
    women_weekend = adjust_demands(women_weekend, 18)
    ...
    # 女假日班
    # women_weekend = [2,0,2,2,3,2,2,0,3,0,0,2,0,0] # input
    weekend_time = ['0700', '0730', '0800', '0830', '0900', '0930', '1000', '1030', '1100', '1130', '1200', '1230',
                    '1300', '1330']
    weekend_time_ls = []
    ...
    weekend_group_1 = girls[:18]
    weekend_group_2 = girls[18:36]
    weekend_group_3 = girls[36:54]
    weekend_group_4 = girls[54:]
    weekend_group = [weekend_group_1, weekend_group_2, weekend_group_3, weekend_group_4]

    # weekend = [0,6,7,13,14,20,21,27]

    for i in range(8):
        random.shuffle(weekend_time_ls)

        # make sure that "同一職場，同一時段，不可只有一個女生值班"
        group_3 = (i % 4 == 2)
        if (weekend_time_ls[-1] != weekend_time_ls[-2]) & group_3:
            for j in range(16):
                if (weekend_time_ls[j] == weekend_time_ls[-2]):
                    tmp = weekend_time_ls[-1]
                    weekend_time_ls[-1] = weekend_time_ls[j]
                    weekend_time_ls[j] = tmp
                    break

        df.iloc[weekends[i], weekend_group[i % 4]] = weekend_time_ls
    ...
    # 男大夜班
    for j in range(92, 117):  # 高雄16人+台北9人
        if j == 92:
            df.iloc[j - 92, j] = '0030'
        else:
            df.iloc[j - 93, j] = '休'
    ...
    # 台中都排1630
    df.iloc[weekdays, taichung_men] = '1630'
    df.iloc[weekends, taichung_men] = '休'
    ...
    for i in range(92, 103):  # 調11個
        if df.iloc[weekends[1], i] != '休' and df.iloc[weekends[1], i] != '0030':
            df.iloc[weekends[1], i] = weekend_time_ls[i - 92]
        if df.iloc[weekends[2], i] != '休' and df.iloc[weekends[2], i] != '0030':
            df.iloc[weekends[2], i] = weekend_time_ls[i - 92]

    for i in range(103, 114):  # 調11個
        if df.iloc[weekends[3], i] != '休' and df.iloc[weekends[3], i] != '0030':
            df.iloc[weekends[3], i] = weekend_time_ls[i - 103]
        if df.iloc[weekends[4], i] != '休' and df.iloc[weekends[4], i] != '0030':
            df.iloc[weekends[4], i] = weekend_time_ls[i - 103]

    for i in range(114, 117):  # 調3個
        if df.iloc[weekends[5], i] != '休' and df.iloc[weekends[5], i] != '0030':
            df.iloc[weekends[5], i] = weekend_time_ls[i - 114]
        if df.iloc[weekends[6], i] != '休' and df.iloc[weekends[6], i] != '0030':
            df.iloc[weekends[6], i] = weekend_time_ls[i - 114]

    for i in range(92, 100):  # 調8個
        if df.iloc[weekends[5], i] != '休' and df.iloc[weekends[5], i] != '0030':
            df.iloc[weekends[5], i] = weekend_time_ls[i - 89]
        if df.iloc[weekends[6], i] != '休' and df.iloc[weekends[6], i] != '0030':
            df.iloc[weekends[6], i] = weekend_time_ls[i - 89]

    for i in range(100, 111):  # 調11個
        if df.iloc[weekends[7], i] != '休' and df.iloc[weekends[7], i] != '0030':
            df.iloc[weekends[7], i] = weekend_time_ls[i - 100]
        if df.iloc[weekends[0], i] != '休' and df.iloc[weekends[0], i] != '0030':
            df.iloc[weekends[0], i] = weekend_time_ls[i - 100]

    for i in range(len(weekends)):
        for j in range(92, 117):
        ...
        for i in range(28):
            if (df.iloc[i, j] == '休') & (not i_2_exist):
                ...
                if i_2 - i_1 <= 7:
                    ...
                    if df.iloc[i - 2, j] == 0:
                        df.iloc[i - 2, j] = '休'
                    elif df.iloc[i - 3, j] == 0:
                        df.iloc[i - 3, j] = '休'
                    else:
                        i_minus = random.randint(4, 7)
                        df.iloc[i - i_minus, j] = '休'
                elif i_2 - i_1 == 10:
                    if df.iloc[i - 3, j] == 0:
                        df.iloc[i - 3, j] = '休'
                    elif df.iloc[i - 4, j] == 0:
                        df.iloc[i - 4, j] = '休'
                    else:
                        i_minus = random.randint(5, 7)
                        df.iloc[i - i_minus, j] = '休'
                elif i_2 - i_1 == 11:
                    if df.iloc[i - 4, j] == 0:
                        df.iloc[i - 4, j] = '休'
                    elif df.iloc[i - 5, j] == 0:
                        df.iloc[i - 5, j] = '休'
                    else:
                        i_minus = random.randint(6, 7)
                        df.iloc[i - i_minus, j] = '休'
                elif i_2 - i_1 == 12:
                    if df.iloc[i - 5, j] == 0:
                        df.iloc[i - 5, j] = '休'
    ...
    ## 補足八天假
    rest_n_diff = 8 * 25 - (df.iloc[:, taipei_kao] == '休').sum(axis=0).sum()

    rest_daily = (df.iloc[:, taipei_kao] == '休').sum(axis=1).values
    available_rest = []
    # 補到單日四人休
    for i in range(28):
        if rest_daily[i] == 1:
    ...
    rest_by_person = (df.iloc[:, taipei_kao] == '休').sum(axis=0).values
    rest_to_go = []
    for i in range(25):
    ...
    for i, x in enumerate(rest_to_go):
        ...
        if (not find_match) & (i != 0):  # switch
            for j in range(i):
                if df.iloc[rest_storage[j], x] == 0:
                    df.iloc[rest_storage[j], x] = '休'
                    rest_storage.append(rest_storage[j])
                    df.iloc[rest_storage[j], rest_to_go[j]] = 0
                    for key in keys:
                        if (rest_dic[key] != 0) & (df.iloc[key, rest_to_go[j]] == 0):
                            rest_dic[key] -= 1
                            df.iloc[key, rest_to_go[j]] = '休'
                            rest_storage[j] = key
                            find_match = True
                            break
                    if find_match:
                        ...
                        df.iloc[rest_storage[j], x] = 0
                        df.iloc[rest_storage[j], rest_to_go[j]] = '休'
                        rest_storage = rest_storage[:-1]
    ...
    # shuffle columns
    df.iloc[:, taipei_kao] = df.reindex(columns=np.random.permutation(df.columns.values[taipei_kao])).values

    # make sure that 1400 meeting is possible (k2 men -> 1330) -> run linear search of first four weekday rows
    men_early = []  # search and store index
    early_meeting_time = '1400'
    for i in range(1, 5):
        for j in taipei_kao:
            if (df.iloc[i, j] == '1330') & (j not in men_early):
                men_early.append(j)
    if len(men_early) < 3:
        early_meeting_time = '1430'
        for i in range(1, 5):
            for j in taipei_kao:
                if (df.iloc[i, j] == '1400') & (j not in men_early):
                    men_early.append(j)
    if len(men_early) < 3:
        early_meeting_time = '1500'
        for i in range(1, 5):
            for j in taipei_kao:
                if (df.iloc[i, j] == '1430') & (j not in men_early):
                    men_early.append(j)
    k2_men = [102, 106, 107]
    for i in range(3):  # switch columns
        switch_index = [k2_men[i], men_early[i]]
        df.iloc[:, switch_index[::-1]] = df.reindex(columns=switch_index).values
    ...
    for i in range(28):
        if i in weekdays:
            for j in range(len(group)):
                ...
                if (len(inner_group[0]) == 4):
                    if (inner_group.count(inner_group[0]) >= 7):
                        meeting_time[i, j] = inner_group[0]
                    elif j == 1:
                        meeting_time[i, j] = early_meeting_time
                    else:
                        meeting_time[i, j] = '1700'
                else:  # 因為可能第一個人放假
                    if (inner_group.count(inner_group[1]) >= 7):
                        meeting_time[i, j] = inner_group[1]
                    elif j == 1:
                        meeting_time[i, j] = early_meeting_time
                    else:
                        meeting_time[i, j] = '1700'
    ...
    if overall_score > score_threshold:
        ...
        best_shift_table.to_excel('../一般班表/新班表_黑白.xlsx')
        best_meeting_percentage.to_excel('../一般班表/各組開會出席率.xlsx')
        best_meeting_time.to_excel('../一般班表/各組開會時間.xlsx')
        best_meeting_act.to_excel('../一般班表/各組開會出席人數.xlsx')
